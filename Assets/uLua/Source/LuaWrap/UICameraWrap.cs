...
public class UICameraWrap
{
	...
	{
		LuaScriptMgr.Push(L, UICamera.GetAnyKeyDown);
		return 1;
	}
	...
	{
		LuaScriptMgr.Push(L, UICamera.onSchemeChange);
		return 1;
	}
	...
	{
		LuaScriptMgr.Push(L, UICamera.onNavigate);
		return 1;
	}
	...
	{
		LuaScriptMgr.Push(L, UICamera.onPan);
		return 1;
	}
	...
	{
		LuaScriptMgr.Push(L, UICamera.lastEventPosition);
		return 1;
	}
	...
	static int set_GetAnyKeyDown(IntPtr L)
	{
		...
		if (funcType != LuaTypes.LUA_TFUNCTION)
		{
			UICamera.GetAnyKeyDown = (UICamera.GetAnyKeyFunc)LuaScriptMgr.GetNetObject(L, 3, typeof(UICamera.GetAnyKeyFunc));
		}
		else
		{
			LuaFunction func = LuaScriptMgr.ToLuaFunction(L, 3);
			UICamera.GetAnyKeyDown = () =>
			{
	...
	static int set_onSchemeChange(IntPtr L)
	{
		...
		if (funcType != LuaTypes.LUA_TFUNCTION)
		{
			UICamera.onSchemeChange = (UICamera.OnSchemeChange)LuaScriptMgr.GetNetObject(L, 3, typeof(UICamera.OnSchemeChange));
		}
		else
		{
			LuaFunction func = LuaScriptMgr.ToLuaFunction(L, 3);
	...
	static int set_onNavigate(IntPtr L)
	{
		...
		if (funcType != LuaTypes.LUA_TFUNCTION)
		{
			UICamera.onNavigate = (UICamera.KeyCodeDelegate)LuaScriptMgr.GetNetObject(L, 3, typeof(UICamera.KeyCodeDelegate));
		}
		else
		{
			LuaFunction func = LuaScriptMgr.ToLuaFunction(L, 3);
			UICamera.onNavigate = (param0, param1) =>
			{
				int top = func.BeginPCall();
				LuaScriptMgr.Push(L, param0);
				LuaScriptMgr.Push(L, param1);
	...
	static int set_onPan(IntPtr L)
	{
		...
		if (funcType != LuaTypes.LUA_TFUNCTION)
		{
			UICamera.onPan = (UICamera.VectorDelegate)LuaScriptMgr.GetNetObject(L, 3, typeof(UICamera.VectorDelegate));
		}
		else
		{
			LuaFunction func = LuaScriptMgr.ToLuaFunction(L, 3);
			UICamera.onPan = (param0, param1) =>
			{
				int top = func.BeginPCall();
				LuaScriptMgr.Push(L, param0);
				LuaScriptMgr.Push(L, param1);
	...
	{
		UICamera.lastEventPosition = LuaScriptMgr.GetVector2(L, 3);
		return 0;
	}
	...
	{
		UICamera.controllerNavigationObject = (GameObject)LuaScriptMgr.GetUnityObject(L, 3, typeof(GameObject));
		return 0;
	}
	...
	static int Raycast(IntPtr L)
	{
		...
		else if (count == 1 && LuaScriptMgr.CheckTypes(L, 1, typeof(UICamera.MouseOrTouch)))
		{
			UICamera.MouseOrTouch arg0 = (UICamera.MouseOrTouch)LuaScriptMgr.GetLuaObject(L, 1);
