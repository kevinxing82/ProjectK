...
public class UIToggleWrap
{
	public static void Register(IntPtr L)
	{
		...
		LuaField[] fields = new LuaField[]
		{
			...
			new LuaField("isColliderEnabled", get_isColliderEnabled, null),
		};
	...
	static int get_isColliderEnabled(IntPtr L)
	{
		...
		if (obj == null)
		{
			...
			if (types == LuaTypes.LUA_TTABLE)
			{
				LuaDLL.luaL_error(L, "unknown member name isColliderEnabled");
			}
			else
			{
				LuaDLL.luaL_error(L, "attempt to index isColliderEnabled on a nil value");
			}
		}

		LuaScriptMgr.Push(L, obj.isColliderEnabled);
		return 1;
	}
	...
	static int set_tween(IntPtr L)
	{
		...
		obj.tween = (UITweener)LuaScriptMgr.GetUnityObject(L, 3, typeof(UITweener));
		return 0;
	}
